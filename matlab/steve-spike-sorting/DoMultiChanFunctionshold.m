...
case 'BuildFilters'
	...
	multiindx=getuprop(h,'multiindx');
	noisefiles=getuprop(h,'noisefiles');
	[filters,subrange,sv,wave] = multiBFI(getuprop(h,'spikefiles'),noisefiles,...
...
case 'Cluster'
	...
	if (get(findobj(h,'Tag','DefaultFiltersBox'),'Value') == 1)
		basefilter = getuprop(h,'DefaultFilters');	% Use default filters
		snipsize=getuprop(h,'snipsize');
		basesubrange = getuprop(h,'DefaultSubrange');
...
case 'Sep'
	...
	nnoise = str2num(get(findobj(h,'Tag','NumNoise'),'String'));
	% Get the waveforms
	[filters,subrange,sv,wave] = multiBFI(getuprop(h,'spikefiles'),getuprop(h,'noisefiles'),...
...
case 'Crosstalk'
	...
	setuprop(h,'plotidxrem',idxrem);
	hmain=getuprop(h,'hmain');
	setuprop(h,'ctchannels',ctchannels);
...
case 'Done'
	...
	scanrate==getuprop (h,'scanrate');
	tmpindx = getuprop(h,'clflindx');
	t = getuprop(h,'t');
	params = getuprop(h,'params');
	scanrate = getuprop(h,'scanrate');	
	k=getuprop(h,'chindices'); k= k(1);
	hmain=getuprop (h,'hmain');
	ctchannels=getuprop(hmain,'ctchannels');
	chanclust=getuprop(hmain,'chanclust');
	outfile=getuprop (hmain,'outfile');
	removedCT=getuprop (hmain,'removedCT');
	plottimes=getuprop(hmain,'plottimes');
	hcc=getuprop(h,'hcc');
	deffilters=getuprop (hmain,'deffilters');
	allchannels=getuprop (hmain,'allchannels');
	...
	set(hmain,'UserData',''); 
	setuprop (hmain,'removedCT',removedCT);
	setuprop (hmain,'chanclust',chanclust);
	setuprop(hmain,'plottimes',plottimes);
	makearrayplot (hmain)
