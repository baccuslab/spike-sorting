function [record,header] = ReadSpikeFile(filename,nrecs)
% [record,header] = ReadSpikeFile(filename,nrecs)
%
...
if (header.format ==1)	% begin reading format 1
	...
	for i = 1:header.NRecords
		...
		record{i} = ReadRecordFormat1(fid,header.NumberChannels);
	end
...
function record = ReadRecordFormat1(fid,nChannels)
...
for i = 1:nChannels	% should this be 63 or NChannels?
