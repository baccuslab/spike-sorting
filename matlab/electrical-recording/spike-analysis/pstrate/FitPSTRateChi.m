function [pbest,chi2,dof,C] = FitPSTRateChi(tsplit,mn,err,keep,trange,pvarstart,varloc,pfix,fixloc,nparms,intratefunc,parmcheckfunc)
...
pbest = pvarstart;
[chi2v,dofv,alpha,beta,A] = PSTRateMrqCoef(tsplit,mn,err,keep,trange,pbest,varloc,pfix,fixloc,nparms,intratefunc);
chi2 = sum(chi2v);
dof = sum(dofv);
if (dof <= nvar)
...
while (ochi2-chi2 > tol*(ochi2+chi2)/2 & iter<itermax)
	...
	while (~accept & iter<itermax)
		...
		if (rcond(alphap) < condtol)
			...
			dp = V*diag(dsinv)*U'*beta;
		else
			dp = alphap\beta;
		end
		if (feval(parmcheckfunc,untangle(pbest+dp,varloc,dims)))
			[chi2v,dofv,alphanew,betanew,A] = PSTRateMrqCoef(tsplit,mn,err,keep,trange,pbest+dp,varloc,pfix,fixloc,nparms,intratefunc);
			chi2 = sum(chi2v);
			dof = sum(dofv);
		else
			chi2 = ochi2 + 1;
...
if (iter == itermax)
	...
	%C = inv(alpha)
	%return;
	% Compute the inverse in a robust way
	[U,S,V] = svd(A);	% Do it on A rather than alpha to build in positive-definiteness
	dS = diag(S);
	maxS = max(dS);
	goodindx = find(dS > sqrt(svdtol)*maxS);
	if (length(goodindx) < length(dS))
		fprintf('There are %d bad parameters: ',length(dS)-length(goodindx));
		fprintf(' %g ',dS);
		fprintf('\n');
		%dS(setdiff(1:nvar,goodindx))
		%dS
	end
	dsinv(goodindx) = 1./dS(goodindx);
	if (length(dsinv) < nvar)
	...
	C = U*diag(dsinv.^2)*U';
	% [sqrt(diag(C)),sqrt(diag(inv(alpha)))]
	% Kill those parameters that were not determined
	badindx = find(diag(C) < sqrt(svdtol));
	%pbest(badindx) = NaN;
end
